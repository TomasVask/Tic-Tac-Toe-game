...
const cpuClickX = function () {
    ...
    initialTurn++;
    turnDisplaySwitch();
    console.log(`RANDOM -- NextTurn ${initialTurn}, CPU pridejo: ${excludeArray}`);
    gameCheck();
};

// const cpuClickZero = async function () {
//     const result = getRandomWithExclude(excludeArray);
//     if (!excludeArray.includes(result)) {
//         excludeArray.push(result);
//     };
//     let newGrid = gridElement[result];
//     if (newGrid.classList) {
//         newGrid.classList.add("clicked");
//         newGrid.classList.replace("normal", "clickedZero");
//     };
//     initialTurn++;
//     turnDisplaySwitch();
//     console.log(`RANDOM -- NextTurn ${initialTurn}, CPU pridejo: ${excludeArray}`);
...
const cpuPreventClicker = function (input) {
    ...
    // if (x.xP1) {
    //     input.classList.replace("normal", "clickedZero");
    // } else {
...
async function preventScenarios(grA, grB, grC, clc) {
    if (grA.classList[1] === clc && grB.classList[1] === clc && grC.classList[1] !== "clickedX"
        || grA.classList[1] === clc && grC.classList[1] === clc && grB.classList[1] !== "clickedX"
        || grB.classList[1] === clc && grC.classList[1] === clc && grA.classList[1] !== "clickedX") {
...
const cpuRunnerX = async function () {
    preventScenarios(grid1, grid2, grid3, "clickedZero");
    preventScenarios(grid1, grid4, grid7, "clickedZero");
    preventScenarios(grid1, grid5, grid9, "clickedZero");
    preventScenarios(grid2, grid5, grid8, "clickedZero");
    preventScenarios(grid3, grid6, grid9, "clickedZero");
    preventScenarios(grid3, grid5, grid7, "clickedZero");
    preventScenarios(grid4, grid5, grid6, "clickedZero");
    preventScenarios(grid7, grid8, grid9, "clickedZero");
};
...
grid9.addEventListener("click", grid9Listener);
